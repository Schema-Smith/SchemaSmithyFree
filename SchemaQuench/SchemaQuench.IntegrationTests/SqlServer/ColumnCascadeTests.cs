...
public class ColumnCascadeTests
{
    ...
    public void EnvTier_FalseVetoesColumnDrop_AbsentAllowsDrop()
    {
        ...
        lock (FactoryContainer.SharedLockObject)
        {
            ...
            try
            {
                ...
                FactoryContainer.Resolve<IConfigurationRoot>()["DropColumnsRemovedFromProduct"] = "false";
                RunSchemaQuench();
                ...
                FactoryContainer.Resolve<IConfigurationRoot>()["DropColumnsRemovedFromProduct"] = string.Empty;
                _environment.ClearReceivedCalls();
                RunSchemaQuench();
    ...
    {
        var path = Path.Combine(packageDir, "Templates", "Main", "Tables", "dbo.CascColTable.json");
        var json = JsonNode.Parse(File.ReadAllText(path))!.AsObject();
        json["DropColumnsRemovedFromProduct"] = value;
        File.WriteAllText(path, json.ToJsonString(new JsonSerializerOptions { WriteIndented = true }));
    }
    ...
    private static bool ColumnExists(System.Data.IDbCommand cmd, string columnName)
    {
        // COLUMNPROPERTY returns NULL when the column does not exist; non-NULL means it exists.
        cmd.CommandText = $"SELECT CAST(CASE WHEN COLUMNPROPERTY(OBJECT_ID('dbo.CascColTable'), '{columnName}', 'AllowsNull') IS NOT NULL THEN 1 ELSE 0 END AS BIT)";
        return Convert.ToBoolean(cmd.ExecuteScalar());
    }

    private static void DropCascColTable(System.Data.IDbCommand cmd)
    {
        cmd.CommandText = "DROP TABLE IF EXISTS [dbo].[CascColTable];";
        cmd.ExecuteNonQuery();
    }
}
