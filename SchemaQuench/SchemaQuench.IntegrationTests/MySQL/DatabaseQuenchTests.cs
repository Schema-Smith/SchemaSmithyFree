...
public class DatabaseQuenchTests
{
    ...
    public void CompletedMigrationScripts_CanTrackScriptExecution()
    {
        ...
        command.CommandText = $@"
            INSERT INTO `{FixtureSetup.MainDb}`.`SchemaSmith_CompletedMigrationScripts`
        ...
        command.CommandText = $@"
            SELECT COUNT(*) FROM `{FixtureSetup.MainDb}`.`SchemaSmith_CompletedMigrationScripts`
            WHERE `ProductName` = '{testProductName}'";
        var count = Convert.ToInt32(command.ExecuteScalar());
        Assert.That(count, Is.EqualTo(1));

        // Cleanup
        command.CommandText = $"DELETE FROM `{FixtureSetup.MainDb}`.`SchemaSmith_CompletedMigrationScripts` WHERE `ProductName` = '{testProductName}'";
        command.ExecuteNonQuery();
    }
    ...
    public void SchemaIdentificationScript_ReturnsDatabase()
    {
        ...
        command.CommandText = $"SELECT '{FixtureSetup.MainDb}' AS DatabaseName";
        using var reader = command.ExecuteReader();

        Assert.That(reader.Read(), Is.True);
        Assert.That(reader.GetString(0), Is.EqualTo(FixtureSetup.MainDb));
    }
    ...
    {
        using var command = _connection.CreateCommand();
        var mainDb = FixtureSetup.MainDb;
    ...
    public void CanSwitchDatabaseContext()
    {
        using var command = _connection.CreateCommand();
        var mainDb = FixtureSetup.MainDb;
        var secondaryDb = FixtureSetup.SecondaryDb;
