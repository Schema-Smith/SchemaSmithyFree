...
/// <summary>
/// Integration tests verifying the per-script "should not apply" sentinel skip on PostgreSQL.
...
public class SentinelSkipIntegrationTests
{
    ...
    public void RunOnceMigration_SentinelSkip_IsRecordedAsCompleted_AndNotReRunOnSecondQuench()
    {
        lock (FactoryContainer.SharedLockObject)
        {
            ...
            try
            {
                ...
                Assert.That(CountCompletedMigrationRows("Before Scripts/SentinelMigration.sql"), Is.EqualTo(1),
    ...
    /// <summary>
    /// An object script (function) that raises the sentinel is skipped and the run still
    /// succeeds — no exit(2), "Skipped (ShouldNotApply)" logged.
    /// </summary>
    [Test]
    public void ObjectScript_SentinelSkip_DoesNotFailRun()
    {
        lock (FactoryContainer.SharedLockObject)
        {
            ...
            try
            {
                ...
                _environment.DidNotReceive().Exit(2);
                _environment.DidNotReceive().Exit(3);
                _progressLog.Received().Info(Arg.Is<string>(s => s.Contains("Skipped (ShouldNotApply)") && s.Contains("public.SentinelFunc.sql")));
            }
            finally
            {
                CleanupSentinelState();
    ...
    public void NonSentinelError_StillFailsRun()
    {
        lock (FactoryContainer.SharedLockObject)
        {
            SetupSharedMocks();
            FactoryContainer.Resolve<IConfigurationRoot>()["SchemaPackagePath"] =
                TestHelper.GetTestProductPath("PostgreSQL", "BeforeTemplateScriptError");
...
DELETE FROM ""SchemaSmith"".""CompletedMigrationScripts"" WHERE ""ProductName"" = '{ProductName}';";
    ...
    private long CountCompletedMigrationRows(string scriptPath)
    {
        ...
        conn.ChangeDatabase(_mainDb);
        using var cmd = conn.CreateCommand();
        cmd.CommandText = $"SELECT COUNT(*) FROM \"SchemaSmith\".\"CompletedMigrationScripts\" WHERE \"ProductName\" = '{ProductName}' AND \"ScriptPath\" = '{scriptPath}'";
    ...
    private long CountMarkerRows()
    {
        ...
        conn.ChangeDatabase(_mainDb);
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT COUNT(*) FROM public.\"SentinelMarker\"";
        var result = Convert.ToInt64(cmd.ExecuteScalar());
        conn.Close();
        return result;
    }
