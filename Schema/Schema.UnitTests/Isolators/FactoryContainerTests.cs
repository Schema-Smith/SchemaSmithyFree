...
public class FactoryContainerTests
{
    ...
    public void ShouldCreateSpecifiedTypeIfInterfaceNotFound()
    {
        lock (FactoryContainer.SharedLockObject)
        {
            var getTest = FactoryContainer.ResolveOrCreate<IFile, FileWrapper>();
            Assert.Multiple(() =>
            {
                Assert.That(getTest, Is.Not.Null);
                Assert.That(getTest, Is.AssignableTo<IFile>());
                Assert.That(getTest, Is.TypeOf<FileWrapper>());
            });
    ...
    public void ShouldCreateAndRegisterSpecifiedTypeIfInterfaceNotFound()
    {
        lock (FactoryContainer.SharedLockObject)
        {
            var getTest = FactoryContainer.ResolveOrCreate<IFile, FileWrapper>(true);
            Assert.Multiple(() =>
            {
                Assert.That(getTest, Is.Not.Null);
                Assert.That(getTest, Is.AssignableTo<IFile>());
            ...
            var getTest2 = FactoryContainer.ResolveOrCreate<IFile>();
            Assert.Multiple(() =>
            {
                Assert.That(getTest2, Is.Not.Null);
                Assert.That(getTest2, Is.AssignableTo<IFile>());
    ...
    public void ShouldReturnNullWhenRequestedTypeAndInterfaceNotFound()
    {
        lock (FactoryContainer.SharedLockObject)
        {
            var getTest = FactoryContainer.Resolve<IFile, FileWrapper>();
            Assert.That(getTest, Is.Null);
        }
    }
    ...
    [Test]
    public void UnregisterShouldRemoveSpecificRegistration()
    {
        lock (FactoryContainer.SharedLockObject)
        {
            var test = new object();
            ...
            FactoryContainer.Unregister<object>();
    ...
    [Test]
    public void ShouldHandleMultipleInterfaceRegistrations()
    {
        lock (FactoryContainer.SharedLockObject)
        {
            ...
            FactoryContainer.Register(intList);
            FactoryContainer.Register(stringList);

            var resolvedInts = FactoryContainer.Resolve<IEnumerable<int>>();
            var resolvedStrings = FactoryContainer.Resolve<IEnumerable<string>>();

            Assert.Multiple(() =>
            {
                Assert.That(resolvedInts, Is.SameAs(intList));
                Assert.That(resolvedStrings, Is.SameAs(stringList));
            });
