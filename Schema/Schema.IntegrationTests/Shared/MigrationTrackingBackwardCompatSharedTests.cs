...
/// <summary>
/// Validates the slice-2 migration of SchemaSmith_CompletedMigrationScripts on the MySQL/MariaDb
/// family. MySQL/MariaDb are excluded from the schema-template fan-out feature itself, but the
/// tracking table still gains the two new columns so the codebase only carries one shape of the
/// table. The MySQL and MariaDb subclasses supply the platform + fixture accessors; every [Test]
/// body here runs on both engines.
/// </summary>
public abstract class MigrationTrackingBackwardCompatSharedTests
{
    ...
    public void TearDown()
    {
        try
        {
            ...
            ForgeKindler.KindleTheForge(_command, Platform, forceReKindle: true);
        }
        finally
        {
            _command.Dispose();
    ...
    public void KindleTheForge_RunTwice_RemainsIdempotent()
    {
        ...
        // force: version-gated kindle would otherwise skip after the fixture's initial kindle
        ForgeKindler.KindleTheForge(_command, Platform, forceReKindle: true);
        Assert.DoesNotThrow(() => ForgeKindler.KindleTheForge(_command, Platform));
