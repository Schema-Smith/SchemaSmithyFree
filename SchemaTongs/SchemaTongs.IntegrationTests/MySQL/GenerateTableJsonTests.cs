...
public class GenerateTableJsonTests
{
    ...
    public void OneTimeSetup()
    {
        ...
        _connectionString = ConnectionString.Build(Platform.MySQL, config["MySQL:Server"], "mysql", config["MySQL:User"], config["MySQL:Password"], config["MySQL:Port"], connProps);
        _integrationDb = GenerateUniqueDBName("GenerateTableJson");

        CreateTestDatabases();

        _testConnectionString = ConnectionString.Build(Platform.MySQL, config["MySQL:Server"], _integrationDb, config["MySQL:User"], config["MySQL:Password"], config["MySQL:Port"], connProps);
    }

    [Test]
    public void ShouldGenerateCorrectJsonForColumns()
    {
        using var conn = DbConnectionFactory.ForPlatform(Platform.MySQL).GetDbConnection(_testConnectionString);
        conn.Open();
...
";
        ...
        AssertColumnProperties(result.Columns[1], "MyInt", "int", true, null);
        ...
        AssertColumnProperties(result.Columns[12], "MySmallint", "smallint", true, null);
        AssertColumnProperties(result.Columns[13], "MyTinyint", "tinyint", true, null);
        AssertColumnProperties(result.Columns[14], "MyBigint", "bigint", true, null);
        ...
        AssertColumnProperties(result.Columns[18], "MyIntWithDefault", "int", false, "42");
...
";
        ...
        var protectedTable = (MySqlTable)PlatformDeserializer.DeserializeTable(protectedJson, Platform.MySQL);
        var unprotectedJson = GenerateTableJson(cmd, _integrationDb, "UnprotectedExtractTable");
        var unprotectedTable = (MySqlTable)PlatformDeserializer.DeserializeTable(unprotectedJson, Platform.MySQL);
    ...
    {
        return (MySqlTable)PlatformDeserializer.DeserializeTable(GenerateTableJson(cmd, schema, table), Platform.MySQL);
    }
