...
- **Libicu-independent runtime** — `InvariantGlobalization` enabled across all CLI tools so the published self-contained binaries run on minimal Linux containers (slim Docker images, hardened distros) that ship without `libicu`. No runtime dependencies on system globalization libraries.
