...
- **Runtime target-version detection and a `MinimumVersion` pre-flight gate.** SchemaSmith now detects each target server's version at deploy time (SQL Server major, PostgreSQL major, MySQL `major.minor`) and uses it to generate version-appropriate DDL where the supported version range diverges. A new optional `MinimumVersion` in `Product.json` declares the product's supported floor: before any deployment, SchemaQuench checks every resolved target and **aborts the whole run** with a manifest naming each server below the floor — no partial deploy. SQL Server accepts either a major (`16`) or the release year (`2022`) as the floor; PostgreSQL and MySQL use the major. Detection failure is a hard error — the run never generates blind against an unknown target version. Cross-platform. — #296
