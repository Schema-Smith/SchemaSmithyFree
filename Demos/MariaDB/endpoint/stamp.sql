-- Ownership-stamp operations for the own-server helper (MariaDB). The caller passes
-- @db and @op via the client's --init-command; connect with NO default database:
--   mariadb ... -N -s --init-command="SET @db='<name>', @op='<check|add|dropIfStamped>'" < stamp.sql
-- MariaDB has no database comments, so the stamp is a marker TABLE created inside each
