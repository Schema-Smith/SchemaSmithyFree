...
# --- Preflight: mysql must be present ---
if (-not (Get-Command mysql -ErrorAction SilentlyContinue)) {
  Write-Error @"
mysql is required but was not found on PATH.
Install the MySQL client tools and re-open your shell:
  Windows : winget install Oracle.MySQL   (or the MySQL Installer 'Client' feature)
  macOS   : brew install mysql-client   (then add its bin to PATH)
  Linux   : apt-get install mysql-client   (or dnf install mysql)
Verify with:  mysql --version
"@
  exit 1
}

# mysql reads the password from MYSQL_PWD (avoids the "password on the command line
...
function Invoke-Mysql([string]$InFile, [string]$Db, [string]$Op) {
  $args = @('-h', $Server, '-P', $Port, '-u', $User, '-N', '-s')
  if ($Db) { $args += @('--init-command', "SET @db='$Db', @op='$Op'") }
  $out = Get-Content $InFile -Raw | & mysql @args
...
$toDrop = @(); $collisions = @()
foreach ($r in $rows) {
  $res = (Invoke-Mysql -InFile "$here/endpoint/stamp.sql" -Db $r.Name -Op 'check') -join ''
...
foreach ($name in $toDrop) { Invoke-Mysql -InFile "$here/endpoint/stamp.sql" -Db $name -Op 'dropIfStamped' | Out-Null }

# --- Bootstrap TestMain (control DB the Initialize templates connect to) ---
Invoke-Mysql -InFile "$here/endpoint/bootstrap.sql" | Out-Null
...
foreach ($r in $rows | Where-Object Type -eq 'product') {
  ...
  Pop-Location
  Invoke-Mysql -InFile "$here/endpoint/stamp.sql" -Db $r.Name -Op 'add' | Out-Null
}
