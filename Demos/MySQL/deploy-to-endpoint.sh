...
command -v mysql >/dev/null 2>&1 || { cat >&2 <<'EOF'
...
mysql_run() { # mysql_run <infile> [<db> <op>]
  local infile="$1"; local db="${2:-}"; local op="${3:-}"
  if [ -n "$db" ]; then
    mysql -h "$SERVER" -P "$PORT" -u "$USER_" -N -s --init-command="SET @db='$db', @op='$op'" < "$infile"
  else
    mysql -h "$SERVER" -P "$PORT" -u "$USER_" -N -s < "$infile"
  fi
}
...
TO_DROP=(); COLLIDE=()
for i in "${!NAMES[@]}"; do
  res="$(mysql_run "$HERE/endpoint/stamp.sql" "${NAMES[$i]}" check)"
...
for name in "${TO_DROP[@]:-}"; do [ -n "$name" ] && mysql_run "$HERE/endpoint/stamp.sql" "$name" dropIfStamped >/dev/null; done
mysql_run "$HERE/endpoint/bootstrap.sql" >/dev/null
...
for i in "${!NAMES[@]}"; do
  ...
  mysql_run "$HERE/endpoint/stamp.sql" "${NAMES[$i]}" add >/dev/null
done
