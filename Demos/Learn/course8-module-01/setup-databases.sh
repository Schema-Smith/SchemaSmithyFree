#!/usr/bin/env bash
# Create the Course 8 Module 1 black-box sandbox database on each sandbox engine, or on
# your own server's single activated engine (LEARN_SERVER): ONE EMPTY database
# diag_blackbox. No schema is seeded -- later tasks deploy a schema into it and break it
...
for engine in $engines; do
  ...
  if [ "$rc" -eq 0 ]; then
    if [ "$reset" -eq 1 ]; then echo "PASS (reset)"; else echo "PASS (black-box sandbox database ready)"; fi
  else
    echo "FAIL"
    echo "$err" | sed 's/^/    /'
    fail=1
...
if [ "$fail" -eq 0 ]; then
  ...
  echo "black-box sandbox database ready on ${where} — empty, ready for schema deploy."
  exit 0
else
