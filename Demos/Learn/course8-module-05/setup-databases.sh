#!/usr/bin/env bash
# Create the Course 8 Module 5 recovery-toolkit sandbox database on each sandbox engine,
# or on your own server's single activated engine (LEARN_SERVER): ONE EMPTY database
# diag_recovery. No schema is seeded -- later tasks deploy a schema into it and break it
# on purpose. Re-running is safe -- creation is idempotent. PASS is reported only after
...
for engine in $engines; do
  ...
  if [ "$rc" -eq 0 ]; then
    if [ "$reset" -eq 1 ]; then echo "PASS (reset)"; else echo "PASS (recovery-toolkit sandbox database ready)"; fi
  else
    echo "FAIL"
    echo "$err" | sed 's/^/    /'
    fail=1
...
if [ "$fail" -eq 0 ]; then
  ...
  echo "recovery-toolkit sandbox database ready on ${where} — empty, ready for schema deploy."
  exit 0
else
