...
for engine in $engines; do
  ...
  if [ "$rc" -eq 0 ]; then
    if [ "$reset" -eq 1 ]; then echo "PASS (reset)"; else echo "PASS (script-slot + data-delivery sandbox database ready)"; fi
  else
    echo "FAIL"
    echo "$err" | sed 's/^/    /'
    fail=1
...
if [ "$fail" -eq 0 ]; then
  ...
  echo "script-slot + data-delivery sandbox database ready on ${where} — empty, ready for schema deploy."
  exit 0
else
