# Create the Course 8 Module 4 script-slot + data-delivery sandbox on each sandbox engine: ONE EMPTY
# database diag_scripts on SQL Server, PostgreSQL, and MySQL. No schema is seeded —
...
function Invoke-Seed {
    param([string]$Engine)
    $file = Join-Path $PSScriptRoot "seed/$Engine/01_create_scripts_database.sql"
    ...
    switch ($Engine) {
        ...
        'mariadb'   { docker cp $file learn-mariadb:/tmp/seed.sql | Out-Null; docker exec learn-mariadb mariadb -uroot -pLearn!Passw0rd -e 'source /tmp/seed.sql' 2>$null | Out-Null }
    }
}

function Get-ScriptsCount {
    param([string]$Engine)
    switch ($Engine) {
        'sqlserver' { (docker exec learn-sqlserver /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P 'Learn!Passw0rd' -C -h -1 -W -Q "SET NOCOUNT ON; SELECT COUNT(*) FROM sys.databases WHERE name = 'diag_scripts'" 2>$null).Trim() }
        'postgres'  { (docker exec learn-postgres psql -U postgres -tAc "SELECT COUNT(*) FROM pg_database WHERE datname = 'diag_scripts'" 2>$null).Trim() }
        'mysql'     { (docker exec learn-mysql mysql -uroot -pLearn!Passw0rd -N -e "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name = 'diag_scripts'" 2>$null).Trim() }
        'mariadb'   { (docker exec learn-mariadb mariadb -uroot -pLearn!Passw0rd -N -e "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name = 'diag_scripts'" 2>$null).Trim() }
    }
}
...
function Initialize-Engine {
    ...
    Invoke-Seed $Engine
    $n = Get-ScriptsCount $Engine
    if ($n -eq '1') { Write-Host "PASS (script-slot + data-delivery sandbox database ready)" }
...
Initialize-Engine 'mariadb'   'MariaDB'

Write-Host ''
if (-not $failed) {
    Write-Host 'script-slot + data-delivery sandbox database ready on all 4 engines — empty, ready for schema deploy.'
    exit 0
} else {
