# Create the Course 8 Module 1 black-box sandbox on each sandbox engine: ONE EMPTY database
# diag_structure on SQL Server, PostgreSQL, and MySQL. No schema is seeded —
...
function Invoke-Seed {
    param([string]$Engine)
    $file = Join-Path $PSScriptRoot "seed/$Engine/01_create_structure_database.sql"
...
function Get-BlackboxCount {
    param([string]$Engine)
    switch ($Engine) {
        'sqlserver' { (docker exec learn-sqlserver /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P 'Learn!Passw0rd' -C -h -1 -W -Q "SET NOCOUNT ON; SELECT COUNT(*) FROM sys.databases WHERE name = 'diag_structure'" 2>$null).Trim() }
        'postgres'  { (docker exec learn-postgres psql -U postgres -tAc "SELECT COUNT(*) FROM pg_database WHERE datname = 'diag_structure'" 2>$null).Trim() }
        'mysql'     { (docker exec learn-mysql mysql -uroot -pLearn!Passw0rd -N -e "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name = 'diag_structure'" 2>$null).Trim() }
        'mariadb'   { (docker exec learn-mariadb mariadb -uroot -pLearn!Passw0rd -N -e "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name = 'diag_structure'" 2>$null).Trim() }
    }
}
...
function Initialize-Engine {
    ...
    Invoke-Seed $Engine
    $n = Get-BlackboxCount $Engine
    if ($n -eq '1') { Write-Host "PASS (structure sandbox database ready)" }
...
Write-Host ''
if (-not $failed) {
    Write-Host 'structure sandbox database ready on all 4 engines — empty, ready for schema deploy.'
    exit 0
} else {
