...
apply_seed() {
  local engine="$1" file="$SCRIPT_DIR/seed/$1/01_create_keys_database.sql"
...
count_keys() {
  local engine="$1"
  case "$engine" in
    sqlserver) docker exec learn-sqlserver bash -c "/opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P 'Learn!Passw0rd' -C -h -1 -W -Q \"SET NOCOUNT ON; SELECT COUNT(*) FROM sys.databases WHERE name = 'diag_keys'\"" 2>/dev/null | tr -d '[:space:]' ;;
    postgres)  docker exec learn-postgres psql -U postgres -tAc "SELECT COUNT(*) FROM pg_database WHERE datname = 'diag_keys'" 2>/dev/null | tr -d '[:space:]' ;;
    mysql)     docker exec learn-mysql mysql -uroot -pLearn!Passw0rd -N -e "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name = 'diag_keys'" 2>/dev/null | tr -d '[:space:]' ;;
    mariadb)   docker exec learn-mariadb mariadb -uroot -pLearn!Passw0rd -N -e "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name = 'diag_keys'" 2>/dev/null | tr -d '[:space:]' ;;
  esac
}
...
seed_engine() {
  ...
  printf '%-12s ' "$label"
  apply_seed "$engine"
  local n; n="$(count_keys "$engine")"
  if [ "$n" = "1" ]; then echo "PASS (index/constraint/FK sandbox database ready)"; else echo "FAIL (found '${n}', expected 1)"; fail=1; fi
}
...
echo
if [ "$fail" -eq 0 ]; then
  echo "index/constraint/FK sandbox database ready on all 4 engines — empty, ready for schema deploy."
  exit 0
else
