...
foreach ($engine in Get-LabEngines) {
    ...
    try {
        ...
        Write-Host $(if ($Reset) { 'PASS (reset)' } else { 'PASS (index/constraint/FK sandbox database ready)' })
    } catch {
        Write-Host 'FAIL'
...
if (-not $failed) {
    ...
    Write-Host "index/constraint/FK sandbox database ready on $where -- empty, ready for schema deploy."
    exit 0
} else {
