...
IF OBJECT_ID('dbo.OrderItem_PriceFix_Backup') IS NULL
    CREATE TABLE dbo.OrderItem_PriceFix_Backup (
...
INSERT INTO dbo.OrderItem_PriceFix_Backup (OrderItemId, OldUnitPrice)
...
WHERE so.OrderDate >= '2026-05-01' AND so.OrderDate < '2026-06-01'
  AND NOT EXISTS (
        SELECT 1
        FROM dbo.OrderItem_PriceFix_Backup b
...
JOIN   dbo.OrderItem_PriceFix_Backup b ON b.OrderItemId = oi.OrderItemId
