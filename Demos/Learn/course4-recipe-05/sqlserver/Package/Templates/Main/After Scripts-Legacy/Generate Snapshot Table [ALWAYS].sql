-- Legacy variant: identical behaviour to the "After Scripts-Modern" copy, built with
-- FOR XML PATH instead of STRING_AGG, which is SQL Server 2017+. OPENJSON stays --
-- it needs compatibility level 130, which the package's declared 2016 floor guarantees.
--
...
SELECT @colDefs = STUFF((SELECT ', ' + '[' + c.col + '] ' + c.dtype + ' NULL'
                           FROM (SELECT REPLACE(REPLACE([Name], '[', ''), ']', '') AS col, [DataType] AS dtype
                                   FROM OPENJSON(@json, '$.Columns') WITH ([Name] NVARCHAR(128) '$.Name', [DataType] NVARCHAR(128) '$.DataType')) c
                           FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 2, ''),
       @colList = STUFF((SELECT ',' + '[' + c.col + ']'
                           FROM (SELECT REPLACE(REPLACE([Name], '[', ''), ']', '') AS col
                                   FROM OPENJSON(@json, '$.Columns') WITH ([Name] NVARCHAR(128) '$.Name')) c
