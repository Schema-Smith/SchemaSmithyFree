...
function Get-TenantCount {
    ...
    switch ($Engine) {
        ...
        'mariadb'   { (docker exec learn-mariadb mariadb -uroot -pLearn!Passw0rd -N -e "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name LIKE 'fleet\_tenant\_%'" 2>$null).Trim() }
    }
}
...
Write-Host ''
if (-not $failed) {
    Write-Host 'All 20 tenant databases created (5 SQL Server, 5 PostgreSQL, 5 MySQL, 5 MariaDB) — empty, ready for Module 1.'
    exit 0
} else {
