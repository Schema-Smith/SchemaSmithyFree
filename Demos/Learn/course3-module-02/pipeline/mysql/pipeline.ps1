...
function Get-IndexState {
    param([string]$Db)
    Invoke-LabSql -Engine 'mysql' -Database $Db -Sql "SELECT IFNULL((SELECT INDEX_NAME FROM information_schema.STATISTICS WHERE TABLE_SCHEMA='$Db' AND INDEX_NAME='IX_Customer_Email' LIMIT 1),'ABSENT')"
}
