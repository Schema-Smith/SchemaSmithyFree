...
function Get-BaselineCount {
    ...
    switch ($Engine) {
        ...
        'mariadb'   { (docker exec learn-mariadb mariadb -uroot -pLearn!Passw0rd -N -e "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name = 'diag_baseline'" 2>$null).Trim() }
    }
}
...
Write-Host ''
if (-not $failed) {
    Write-Host 'diagnostics baseline database ready on all 4 engines — empty, ready for Module 1.'
    exit 0
} else {
