...
GO
-- Legacy variant: identical behaviour to the Functions-Modern copy. Only the
-- dependency-clearing batch differs -- it builds its statement list with
-- FOR XML PATH instead of STRING_AGG, which is SQL Server 2017+.
DECLARE @v_SearchTerm VARCHAR(2000) = '%ufnLeadingZeros%'
DECLARE @v_SQL VARCHAR(MAX) = STUFF((SELECT ';' + CHAR(13) + CHAR(10) + x.Task
