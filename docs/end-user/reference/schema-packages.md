...
| Step | Folder | Quench Slot | Execution Behavior |
|------|--------|-------------|-------------------|
| 1 | `ProductScripts/Before Product` | Product Before | Sequential, tracked (run once unless `[ALWAYS]` in filename) |
| 2 | `MigrationScripts/Before` | Before | Sequential, tracked (run once unless `[ALWAYS]`) |
| 3 | `Schemas` | Objects | Dependency retry loop |
| 4 | `DataTypes` | Objects | Dependency retry loop |
| 5 | `FullTextCatalogs` | Objects | Dependency retry loop |
| 6 | `FullTextStopLists` | Objects | Dependency retry loop |
| 7 | `XMLSchemaCollections` | Objects | Dependency retry loop |
| 8 | `Functions` | Objects | Dependency retry loop |
| 9 | `Views` | Objects | Dependency retry loop |
| 10 | `Procedures` | Objects | Dependency retry loop |
| 11 | _(Table Quench)_ | _(internal)_ | Table JSON applied via TableQuench stored procedure |
| 12 | `MigrationScripts/BetweenTablesAndKeys` | BetweenTablesAndKeys | Sequential, tracked |
| 13 | _(Foreign Key Quench)_ | _(internal)_ | Foreign keys applied after BetweenTablesAndKeys |
| 14 | `MigrationScripts/AfterTablesScripts` | AfterTablesScripts | Sequential, tracked |
| 15 | `Triggers` | AfterTablesObjects | Dependency retry loop |
| 16 | `DDLTriggers` | AfterTablesObjects | Dependency retry loop |
| 17 | `Table Data` | TableData | Dependency retry loop |
| 18 | `MigrationScripts/After` | After | Sequential, tracked |
| 19 | `ProductScripts/After Product` | Product After | Sequential, tracked |

Steps 2--18 execute per template (in `TemplateOrder`), per database (as identified by `DatabaseIdentificationScript`). Steps 1 and 19 execute once per quench run against the server connection.
...
|---|---|
| `Before` | One-time migration scripts that must run before any database objects are created or modified. |
...
| `TableData` | Data population scripts (MERGE statements, seed data). Run after all structural changes and triggers are in place. |
