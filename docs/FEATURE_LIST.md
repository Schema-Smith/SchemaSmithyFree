...
SchemaSmith ships three CLI tools. All three work against all three supported database engines from a single, shared schema-package format.

| Tool         | Description                                       | SQL Server | PostgreSQL | MySQL |
|--------------|---------------------------------------------------|:----------:|:----------:|:-----:|
...
**Database engines:** SQL Server 2017+ (CI runs against 2022); PostgreSQL 15+; MySQL 8.0+.
...
| Feature | SQL Server | PostgreSQL | MySQL | Notes |
|---|---|---|---|---|
...
| Clustered indexes | ✓ | n/a | n/a | `Clustered: true` |
| Non-clustered indexes | ✓ | ✓ | ✓ | Default index shape |
| Filtered / partial indexes | ✓ | ✓ | n/a | `FilterExpression` |
| Covering / INCLUDE indexes | ✓ | ✓ | n/a | PG 11+; `IncludeColumns` |
| Columnstore indexes | ✓ | n/a | n/a | `ColumnStore` flag |
| Full-text indexes | ✓ | n/a | ✓ | SQL Server: 1 per table; MySQL: multiple |
| XML indexes | ✓ | n/a | n/a | `XmlIndexes` array; primary + secondary |
| Spatial indexes | n/a | ✓ | ✓ | PG via `gist`/`spgist` AccessMethod; MySQL via index on spatial column |
| Statistics | ✓ | ✓ | n/a | SQL Server traditional; PG extended (`ndistinct`, `dependencies`, `mcv`) |
| Views | ✓ | ✓ | ✓ | Script folder |
| Indexed views | ✓ | n/a | n/a | `Indexed Views/*.json` |
| Materialized views | n/a | ✓ | n/a | `Materialized Views/*.json` |
...
| DDL triggers | ✓ | n/a | n/a | `DDLTriggers/` folder |
| Schemas | ✓ | ✓ | n/a | `Schemas/` folder; MySQL has no separate schema concept |
| User-defined types (scalar) | ✓ | ✓ | n/a | SQL Server `DataTypes/`; PG `Domain Types/` |
| User-defined enum types | n/a | ✓ | n/a | `Enum Types/` |
| User-defined composite types | n/a | ✓ | n/a | `Composite Types/` |
| User-defined table types | ✓ | n/a | n/a | Bundled with `UserDefinedTypes` ShouldCast flag |
| Full-text catalogs | ✓ | n/a | n/a | `FullTextCatalogs/` |
| Full-text stop lists | ✓ | n/a | n/a | `FullTextStopLists/` |
| XML schema collections | ✓ | n/a | n/a | `XMLSchemaCollections/` |
| Exclude constraints | n/a | ✓ | n/a | `ExcludeConstraints` array |
| Sequences | n/a | ✓ | n/a | `Sequences/` folder |
| Aggregates | n/a | ✓ | n/a | `Aggregates/` folder |
| Rules | n/a | ✓ | n/a | `Rules/` folder |
| Trigger functions | n/a | ✓ | n/a | `Trigger Functions/` |
| Window functions | n/a | ✓ | n/a | `Window Functions/` |
...
| Persisted computed columns | ✓ | n/a | n/a | `Persisted: true` |
| Sparse columns | ✓ | n/a | n/a | `Sparse` column property |
| Dynamic data masking | ✓ | n/a | n/a | `DataMaskFunction` column property |
...
| Data compression | ✓ | n/a | n/a | `CompressionType`: NONE / ROW / PAGE |
| Table-level access method | n/a | ✓ | n/a | `AccessMethod` |
| Persistence type (UNLOGGED / TEMPORARY) | n/a | ✓ | n/a | `PersistenceType` |
| Row-level security | n/a | ✓ | n/a | `RowLevelSecurity`, `ForceRowLevelSecurity` |
| Table fill factor | n/a | ✓ | n/a | `FillFactor` 0–100 on table |
| Per-table `UpdateFillFactor` | ✓ | ✓ | n/a | OR'd with template + index level |
| Temporal tables (system-versioning marker) | ✓ | n/a | n/a | `IsTemporal` flag |
| Change Data Capture marker | ✓ | n/a | n/a | `EnableCDC` flag |
...
| Clustered | ✓ | n/a | n/a | `Clustered` |
| Filtered / partial (`WHERE` predicate) | ✓ | ✓ | n/a | `FilterExpression` |
| Included / covering columns | ✓ | ✓ | n/a | PG 11+; `IncludeColumns` |
| Per-index fill factor | ✓ | n/a | n/a | `FillFactor`, `UpdateFillFactor` |
| Per-index compression | ✓ | n/a | n/a | `CompressionType` |
| Columnstore | ✓ | n/a | n/a | `ColumnStore` |
| Index access method (btree / gin / gist / brin / spgist / hash) | n/a | ✓ | n/a | `AccessMethod` |
| Index type `BTREE` / `HASH` | n/a | n/a | ✓ | `IndexType` |
| Visible / invisible index | n/a | n/a | ✓ | `Visible` flag |
| Index tablespace | n/a | ✓ | n/a | `Tablespace` |
| Sort direction per column | ✓ | ✓ | ✓ | In `IndexColumns` string |
| Conditional index application | ✓ | ✓ | ✓ | `ShouldApplyExpression` |
| XML index (primary / secondary VALUE / PATH / PROPERTY) | ✓ | n/a | n/a | `XmlIndexes` |
| Full-text index — full props (catalog, stop list, change tracking, key index) | ✓ | n/a | n/a | `FullTextIndex` (1 per table) |
...
| FK action `CASCADE` | ✓ | ✓ | ✓ | |
| FK action `SET NULL` | ✓ | ✓ | ✓ | |
| FK action `SET DEFAULT` | ✓ | ✓ | n/a | MySQL InnoDB does not support |
...
| Exclude constraint | n/a | ✓ | n/a | `ExcludeConstraints` |
| Deferrable / initially deferred (exclude) | n/a | ✓ | n/a | `Deferrable`, `InitiallyDeferred` |
...
| Stored procedures | ✓ | ✓ | ✓ | |
| Scalar functions | ✓ | ✓ | ✓ | |
| Table-valued / set-returning functions | ✓ | ✓ | n/a | |
| Aggregate functions | n/a | ✓ | n/a | `Aggregates/` |
| Window functions | n/a | ✓ | n/a | `Window Functions/` |
| Trigger functions (separate from triggers) | n/a | ✓ | n/a | `Trigger Functions/` |
| Views | ✓ | ✓ | ✓ | |
| Indexed views (diff-based) | ✓ | n/a | n/a | Index-only changes skip view rebuild |
| Materialized views (diff-based) | n/a | ✓ | n/a | Index-only changes skip MV rebuild |
| DML triggers (BEFORE / AFTER / INSTEAD OF) | ✓ | ✓ | ✓ | AfterTablesObjects slot |
| DDL triggers | ✓ | n/a | n/a | |
| Rules | n/a | ✓ | n/a | `Rules/` |
...
| Per-table / per-index `UpdateFillFactor` | ✓ | ✓ | n/a | OR'd with template setting |
| `Template.Required` | ✓ | ✓ | ✓ | Fail if 0 databases returned |
| `Template.SkipIfReadOnly` | ✓ | ✓ | ✓ | AG secondary / replica handling |
| `VerboseLogging` | ✓ | n/a | n/a | SQL Server `InfoMessage`; PG/MySQL surface notices by default |
...
| `MERGE` script generation | ✓ | ✓ | n/a | PG 15+ |
...
| Rule disable wrapping | n/a | ✓ | n/a | `DisableRules` |
| Partition-descendant control (`MERGE ... ONLY`) | n/a | ✓ | n/a | `UpdateDescendents` |
| Identity insert handling | ✓ | n/a | n/a | `SET IDENTITY_INSERT` wrapping |
| `OVERRIDING SYSTEM VALUE` on identity | n/a | ✓ | n/a | |
| Geometry / geography (WKT round-trip) | ✓ | n/a | ✓ | MySQL: basic spatial via WKT |
| HierarchyID round-trip | ✓ | n/a | n/a | |
| XML / NTEXT / TEXT / IMAGE handling | ✓ | n/a | n/a | |
| JSON / JSONB round-trip | n/a | ✓ | n/a | |
...
| Token resolution / `TokenizeScripts` | ✓ | n/a | n/a | DataTongs tokenization flag is SQL Server only |
...
| `<*QueryFile*>` tag | ✓ | ✓ | ✓ | |
| `<*SpecificTable*>` tag | ✓ | ✓ | ✓ | |
| `<*SpecificIndexedView*>` tag | ✓ | n/a | n/a | |
| `<*SpecificMaterializedView*>` tag | n/a | ✓ | n/a | |
| Auto `{{ProductName}}` / `{{TemplateName}}` | ✓ | ✓ | ✓ | |
| Auto `{{TableSchema}}` | ✓ | ✓ | ✓ | Template-wide table JSON |
| Auto `{{IndexedViewSchema}}` | ✓ | n/a | n/a | |
| Auto `{{MaterializedViewSchema}}` | n/a | ✓ | n/a | |
...
| Windows authentication (blank user/pass) | ✓ | n/a | n/a | |
...
| `VerboseLogging` setting | ✓ | n/a | n/a | SQL Server `InfoMessage` only; PG/MySQL surface notices by default |
