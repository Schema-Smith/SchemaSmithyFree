...
WHEN MATCHED AND (NOT (Target.[Comments] = Source.[Comments] OR (Target.[Comments] IS NULL AND Source.[Comments] IS NULL)) AND NOT (Target.[EmailAddress] = Source.[EmailAddress] OR (Target.[EmailAddress] IS NULL AND Source.[EmailAddress] IS NULL)) AND NOT (Target.[ModifiedDate] = Source.[ModifiedDate] OR (Target.[ModifiedDate] IS NULL AND Source.[ModifiedDate] IS NULL)) AND NOT (Target.[ProductID] = Source.[ProductID] OR (Target.[ProductID] IS NULL AND Source.[ProductID] IS NULL)) AND NOT (Target.[Rating] = Source.[Rating] OR (Target.[Rating] IS NULL AND Source.[Rating] IS NULL)) AND NOT (Target.[ReviewDate] = Source.[ReviewDate] OR (Target.[ReviewDate] IS NULL AND Source.[ReviewDate] IS NULL)) AND NOT (Target.[ReviewerName] = Source.[ReviewerName] OR (Target.[ReviewerName] IS NULL AND Source.[ReviewerName] IS NULL))) THEN
  UPDATE SET
...
ALTER TABLE [Production].[ProductReview] ENABLE TRIGGER ALL;
